...
use syn::{
    ...
    token::Plus,
    Arm, Expr, LitInt, Pat, Token,
};

/// Test documentation for materialize index
#[proc_macro]
pub fn create_hybrid_index(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    for (i, layer) in desc.layer_types.iter().enumerate() {
        ...
        let layer_type = parse_str::<TokenStream>(&format!(
            "::clearned::private::{}<K, {}>",
            layer_name, param
        ))
        .expect("");
    ...
    for (arm, index) in desc.layout.iter().cloned() {
        ...
        let layer_type =
            parse_str::<TokenStream>(&format!("::clearned::private::{}", name)).expect("");
    ...
    let mut layer = quote! {
        pub enum #layer_name<K: ::clearned::private::Key> {
            #(#variants),*
        }

        impl<K: ::clearned::private::Key> ::clearned::private::HybridLayer<K> for #layer_name<K> {
            fn len(&self) -> usize {
                use ::clearned::private::NodeLayer;
            ...
            fn search(&self, key: &K, range: ::clearned::private::ApproxPos) -> ::clearned::private::ApproxPos {
                use ::clearned::private::InternalLayer;
            ...
            fn build(layer: usize, base: impl ExactSizeIterator<Item = K>) -> Self {
                use ::clearned::private::InternalLayerBuild;
            ...
            fn load(layer: usize, path: impl AsRef<std::path::Path>) -> ::clearned::private::Result<Self>
            where
                Self: Sized {
                use ::clearned::private::InternalLayerBuild;
    ...
    let index = quote! {
        #[allow(unused_import)]
        pub struct #name<K: ::clearned::private::Key, V: ::clearned::private::Value>(::clearned::private::HybridIndex<K, V, #layer_name<K>>);

        impl<K: ::clearned::private::Key, V: ::clearned::private::Value> ::clearned::private::ImmutableIndex<K, V> for #name<K, V> {
            fn build_in_memory(base: impl ExactSizeIterator<Item = (K, V)>) -> Self {
                Self(::clearned::private::HybridIndex::build_in_memory(base))
            }
            ...
            fn build_on_disk(
                ...
                Ok(Self(::clearned::private::HybridIndex::build_on_disk(base, path, threshold)?))
            }

            fn load(path: impl AsRef<::std::path::Path>, threshold: usize) -> ::clearned::private::Result<Self> {
                Ok(Self(::clearned::private::HybridIndex::load(path, threshold)?))
            }
            ...
            type RangeIterator<'e> = ::clearned::private::HybridIndexRangeIterator<'e, K, V>;
        }
    };
